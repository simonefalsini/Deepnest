...
namespace boost {
    namespace qvm {
        template<>
        struct is_scalar<libnfporb::LongDouble> {
...
const coord_t MIN_COORD = std::numeric_limits<coord_t>::lowest();
...
TranslationVector trimVector(const polygon_t::ring_type& rA, const polygon_t::ring_type& rB, const TranslationVector& tv) {
	...
	coord_t shortest = bg::length(segment_t{{0,0},tv.vector_});
	...
	for(const auto& ptA : rA) {
		...
		for(const auto& pti : intersections) {
			...
			len = bg::length(segi);
			if(smaller(len, shortest) && larger(len, NFP_EPSILON)) {
